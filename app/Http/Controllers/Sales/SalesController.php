...

{
    ...
    {
        $getCustomers = ConsumerLogin::all();
        return view('sales/salesForm',compact(['getCustomers']));
...

    {
        ...
        $exists = ProductAttribute::where('imei', $barcode)->exists();
        $products = 0;
        if ($exists) {

            $attribute = ProductAttribute::where('imei', $barcode)->first();



        $products = PurchaseDetail::join('products', 'products.product_id', '=', 'purchase_details.product_id')

            ->where('purchase_details.purchase_details_id', $attribute->purchase_details_id)
